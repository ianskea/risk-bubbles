...
def run_backtest(ticker, years=4, initial_capital=10000):
    ...
    df.loc[df['risk_total'] > 0.75, 'position'] = 0.0
    
    # Forward fill position (Hold)
    df['position'] = df['position'].ffill().fillna(0) # Start with 0 if unknown
    
    # 3. Calculate Returns
    df['pct_change'] = df['Close'].pct_change()
    
    # Strategy Return = Position(yesterday) * Return(today)
    # We shift position by 1 because we act on *yesterday's* close signal (or today's signal for tomorrow)
    ...
    print(f"Buy & Hold Final:   ${final_bh:,.2f} ({bh_ret*100:+.1f}%)")
    print(f"Strategy Final:     ${final_strat:,.2f} ({strat_ret*100:+.1f}%)")
